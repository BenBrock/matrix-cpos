...
TEST(DenseMatrixView, RowColumnViews) {
  ...
  for (auto&& [m, n] :
    ...
    for (auto&& [j, column] : mc::columns(view)) {
      EXPECT_EQ(column.size(), view.shape()[0]);
...
TEST(DenseMatrixView, DiagonalViews) {
  ...
  for (auto&& [m, n] :
    ...
    // Iterate through "positive diagonals" (beginning along 0th row)
    ...
    // Iterate through "negative diagonals" (beginning along 0th column)
    ...
    for (auto&& [d, diagonal] : mc::diagonals(view)) {
      std::ptrdiff_t d_neg = d;
